 ...
 * It lets you specify a timeout for how long the cached data is valid. It also
 * gives you programmatic control over key generation, refreshing of the cache
 * and whether the cached content should be served or not.
 * 
 * Usage Example: <%@ taglib prefix="ias" uri="http://mascix.com/taglibs/coherence"
 * %> <ias:cache key="<%= cacheKey %>" usecached="<%= useCached %>"
 * refresh="<%= reload %>" timeout="3600"> ... expensive operation ...
 * </ias:cache>
 */
public class CacheTag extends BodyTagSupport {
	private static final long serialVersionUID = -1636903972059739685L;
	/**
	...
	/**
	 * The key into the cache. This is generated by suffixing the servlet path
	 * with the key if one is specified or by a generated suffix.
	 */
	private String _key;
	...
	/**
	 * This boolean specifies whether the cached response must be sent or the
	 * body should be evaluated. The cache is not refreshed.
	 */
	 ...
	 */
	private static final Logger _logger = Logger.getLogger(CacheTag.class.getName());
	...
	/**
	 * doStartTag is called every time the cache tag is encountered. By the time
	 * this is called, the tag attributes are already set, but the tag body has
	 * not been evaluated. The cache key is generated here and the cache is
	 * obtained as well
	 * 
	 * @throws JspException
	 *             the standard exception thrown
	 * @return EVAL_BODY_INCLUDE when nocache is specified so that the tag body
	 *         is just evaluated into the output stream SKIP_BODY if the cached
	 *         response is valid in which case it is just written to the output
	 *         stream, hence there is nothing more to be done.
	 *         EVAL_BODY_BUFFERED is the default return value which ensures that
	 *         the BodyContent is created and the tag body is evaluated into it.
	 */
	 ...
	 * if nocache is specified in which case this should do nothing if there was
	 * no cached response in which case the response data is obtained from the
	 * bodyContent and cached if the response has expired in which case the
	 * cache is refreshed
	 * 
	 * @throws JspException
	 *             the standard exception thrown
	...
	/**
	 * This is used to set a user-defined key to store the response in the
	 * cache.
	 */
	...
	/**
	 * This sets the time for which the cached response is valid. The cached
	 * entry is invalid after this time is past. If no unit is specified, then
	 * the timeout is assumed to be in seconds. A different unit can be
	 * specified by postfixing the timeout value with the desired unit:
	...
	public void setTimeout(String timeout) {
		if (timeout != null) {
			try {
				...
				int i = 0;
				while (i < timeout.length() && Character.isDigit(timeout.charAt(i)))
					i++;
	...
	/**
	 * This attribute is used to programmatically enable or disable the use of
	 * the cached response. If noCache is true, then the cached response is not
	 * sent, instead the tag body is evaluated and sent out, the cache is not
	 * refreshed either.
	 */
	 ...
	 * If refresh is true, the cached response is not sent, instead the tag body
	 * is evaluated and sent and the cache is refreshed with the new response.
	 */
